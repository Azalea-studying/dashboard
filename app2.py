...
def cumulative_revenue_fig():
    ...
    # 排除Consolidated（避免重复计算），只展示业务单元占比
    unit_df = temp_df[temp_df["Business"] != "Consolidated"]
    fig = px.pie(
        unit_df, values="Revenue", names="Business",
        title="Cumulative Revenue Distribution (Year 0)",
        hole=0.4,  # 环形图更美观
        color_discrete_sequence=["#1f77b4", "#ff7f0e", "#2ca02c"],
...
def balance_sheet_fig():
    """6. 资产负债表摘要（将柱状图改为饼图，展示资产占比）"""
    temp_bs_df = pd.DataFrame({
        "Asset Type": ["Current Assets", "Non-current Assets"],
        "Value": balance_sheet_df.iloc[0:2]["Value"].values
    })
    fig = px.pie(
        temp_bs_df, values="Value", names="Asset Type",
        title="Balance Sheet Asset Distribution (Year 0)",
        color_discrete_sequence=["#1f77b4", "#ff7f0e"],
        hover_data={"Value": ":,.0f"}
    )
    fig.update_traces(textinfo="percent+label")  # 显示占比
    fig.update_layout(template="plotly_white", height=400)
    return fig

def cagr_fig():
    """7. 业务单元CAGR（将柱状图改为横向条形图，减少重复）"""
    ...
    fig = px.bar(
        cagr_df, y="Business Unit", x="CAGR (%)",  # 横向：y为类别，x为数值
...
if __name__ == "__main__":
    # Colab测试：app.run(debug=False)
    # PythonAnywhere部署：app.run_server(debug=False)
